    ...
    <div className="view-inner">
      <div className="view-header">
        <div>
          ...
          <p className="muted">Documents are generated in your browser from the project data — nothing is uploaded anywhere.</p>
        </div>
      </div>
      <h2>Documents (PDF)</h2>
          ...
          <div key={k} className="card col">
            ...
            <div className="row">
              ...
              <button className="btn sm primary" disabled={!!busy} onClick={() => pdf(k)}>
                {busy === `pdf-${k}` ? <Loader2 size={13} className="spin" /> : <FileDown size={13} />} Download PDF
              </button>
            </div>
          </div>
        ))}
      ...
      <div className="row wrap">
        <button className="btn" disabled={!!busy} onClick={() => run('svg', async () => void (ctx && downloadBlob(await exportPlanSvg(doc, ctx.lookup), `${name}-plan.svg`)))}>
          <ImageIcon size={14} /> Scaled plan (SVG, vector)
        </button>
